...
simpar.wk_stdev = 1; % fi_sa                              % stddev of the noise used in acceleration to simulate the robot movement.
...
%simpar.Ak = [1 simpar.T; 0 1];                                % Ak matrix of the KF
simpar.Ak = 1;

simpar.Bk = 1;                                            % Bk matrix of the KF

simpar.Qk = 2 * simpar.odometry_stdev;                    % twice the real uncertainty of the odometry displacement [m]

simpar.Rk = 2 * simpar.sonar_stdev;                       % twice the real uncertainty of the sonar measurement of the position [m]
...
xk_hat = (simpar.Ak * xk_1) + (simpar.Bk * uk);
Pk_hat = (simpar.Ak * Pk_1 * simpar.Ak') + simpar.Qk;

Kk = (Pk_hat * simpar.Hk') * inv((simpar.Hk * Pk_hat * simpar.Hk') + simpar.Rk);

if isempty(zk)
    xk = xk_hat;
    Pk = Pk_hat;
else
   xk = xk_hat + (Kk * (zk - simpar.Hk * xk_hat));
   Pk = (eye(size(Kk * simpar.Hk)) - (Kk * simpar.Hk)) * Pk_hat;
end
...
%       |
% Pe =  Pk - Pk2
% %95 probablity it will between -2sigma 0 2sigma
% 
